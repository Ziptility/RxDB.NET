...
public sealed class MutationResolver<TDocument> where TDocument : class, IReplicatedDocument
{
    ...
    /// <param name="documentService">The document documentService to be used for data access.</param>
...
#pragma warning disable CA1822 // disable Mark members as static since this is a class instantiated by DI
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (conflicts, updates, creates) = await CategorizeDocumentsAsync(documents, documentService, cancellationToken).ConfigureAwait(false);
        ...
        {
            var applyConflicts = await ApplyChangesAsync(creates, updates, documentService, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var document in documents)
        {
            ...
            // Fetch the current state of the document from the documentService
            // This is crucial for detecting conflicts as per the RxDB protocol
            var existing = await documentService.GetDocumentByIdAsync(document.NewDocumentState.Id, cancellationToken)
                .ConfigureAwait(false);

            if (existing != null)
            {
                // Document exists in the documentService, handle potential conflicts
                MutationResolver<TDocument>.HandleExistingDocument(document, existing, conflicts, updates, documentService);
            }
            else
            {
                // Document doesn't exist in the documentService, handle as a new document
    ...
    /// <param name="existing">The existing document in the documentService.</param>
    ...
    private static void HandleExistingDocument(
        ...
        IDocumentService<TDocument> documentService)
    {
        // Check if the assumed master state matches the current state in the documentService
        // This is a key part of the RxDB conflict detection mechanism
        if (document.AssumedMasterState == null || !documentService.AreDocumentsEqual(existing, document.AssumedMasterState))
        {
    ...
    /// <summary>
    /// Applies the changes to the documentService, including creating new documents and updating existing ones.
    ...
    /// <param name="documentService">The document documentService to be used for data access.</param>
    /// <param name="cancellationToken">A cancellation token that can be used to cancel the work.</param>
    private static async Task HandleDocumentUpdateAsync(TDocument update, IDocumentService<TDocument> documentService, CancellationToken cancellationToken)
    {
        if (update.IsDeleted)
        {
            // Handle soft deletes as per RxDB protocol
            // Documents are never physically deleted, only marked as deleted
            await documentService.MarkAsDeletedAsync(update, cancellationToken).ConfigureAwait(false);
        }
        else
        {
            // Update the existing document
            await documentService.UpdateDocumentAsync(update, cancellationToken).ConfigureAwait(false);
        }
    }
}
